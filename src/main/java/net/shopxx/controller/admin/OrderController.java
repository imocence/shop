...
//		model.addAttribute("defaultShippingMethod", defaultShippingMethod);
     ...
     * @param memo               附言
     * @param freight            运费
     * @param amount             订单电子币总额
     * @param couponAmount       订单购物券总额
     ...
     */
    @PostMapping("/update")
    public String update(Long id, Long areaId, Long paymentMethodId, Long shippingMethodId, BigDecimal freight, BigDecimal tax, BigDecimal offsetAmount, BigDecimal offsetCouponAmount, Long rewardPoint, String consignee, String address, String zipCode, String phone, String invoiceTitle, String memo, RedirectAttributes redirectAttributes) {
...
//		if (OrderPayment.Method.deposit.equals(orderPayment.getMethod()) && orderPayment.getAmount().compareTo(member.getBankbookBalance()) > 0
        if (orderPayment.getAmount().compareTo(member.getBankbookBalance()) > 0 && orderPayment.getCouponAmount().compareTo(member.getCouponBalance()) > 0) {
            return ERROR_VIEW;
        }
    ...
    public String refunds(OrderRefunds orderRefunds, Long orderId, String usercode, Long paymentMethodId, RedirectAttributes redirectAttributes) {
        ...
        order.setMember(member);
        if (order.getRefundableAmount().compareTo(BigDecimal.ZERO) <= 0 && order.getRefundableCouponAmount().compareTo(BigDecimal.ZERO) <= 0) {
            return ERROR_VIEW;
        }
